...
class CycleGANModel(nn.Module):
    ...
    def __init__(
        ...
        self.netG_A = networks.define_G(input_nc, output_nc, ngf, netG_name, norm,
                                        not no_dropout, init_type, init_gain, self.gpu_ids)
        self.netG_B = networks.define_G(output_nc, input_nc, ngf, netG_name, norm,
                                        not no_dropout, init_type, init_gain, self.gpu_ids)

        if self.isTrain:  # define discriminators
            self.netD_A = networks.define_D(output_nc, ndf, netD_name,
                                            n_layers_D, norm, init_type, init_gain, self.gpu_ids)
            self.netD_B = networks.define_D(input_nc, ndf, netD_name,
                                            n_layers_D, norm, init_type, init_gain, self.gpu_ids)
